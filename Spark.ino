...
void change_generic_model(char *new_eff, int slot) {
  // Changed presets[5] to [4] to try whether this would work. If not then it needs to be set back to [5]
...
void change_generic_onoff(int slot,bool onoff) {
  ...
  //DEBUG("Slot: ");DEBUG(slot);DEBUG(" changed.");
  spark_process();
  app_process();  
}
